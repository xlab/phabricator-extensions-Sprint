...
final class SprintProjectProfileController
  ...
  protected function shouldShowSprintFields($project)
  {
    if ($project instanceof PhabricatorProject) {
      return (strpos($project->getName(), SprintConstants::MAGIC_WORD) !== FALSE);
    }
  }

  private function getAppUri($project) {
    if (!$this->shouldShowSprintFields($project)) {
      $app_base_uri = '/project/';
    } else {
      $app_base_uri = $this->getApplicationURI();
    }
    return $app_base_uri;
  }


  private function getBoardName($project) {
    if (!$this->shouldShowSprintFields($project)) {
  ...
  public function processRequest() {
    ...
    $project = $this->getProject($viewer);
    $app_base_uri = $this->getAppUri($project);
    $board_name = $this->getBoardName($project);
    ...
    $icon = id(new PHUIIconView())
      ...
      $board_btn = id(new PHUIButtonView())
          ->setTag('a')
          ->setText(pht($board_name))
          ->setHref($app_base_uri.("board/{$id}/"))
          ->setIcon($icon);
  ...
  private function buildActionListView(PhabricatorProject $project) {
    ...
    $view->addAction(
      id(new PhabricatorActionView())
        ...
        ->setIcon('fa-pencil')
        ->setHref($app_base_uri.("edit/{$id}/")));

    $view->addAction(
      id(new PhabricatorActionView())
        ->setName(pht('Edit Members'))
        ->setIcon('fa-users')
        ->setHref($app_base_uri.("members/{$id}/"))
